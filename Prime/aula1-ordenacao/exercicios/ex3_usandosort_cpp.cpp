...
bool compara(int x,
             int y) { // declaro a função bool compara, que recebe dois inteiros
  ...
  return false; // então o capital de x não é maior, logo x e y não estão em
                // ordem
}
...
int main() {
  ...
  for (int i = 1; i <= n; i++)
    scanf("%d", &capital[i]); // leio o capital de cada empres

  for (int i = 1; i <= n; i++)
    empr[i] = i; // faço um vetor com o número de cada empresa

  sort(empr + 1, empr + n + 1,
       compara); // ordeno o vetor pelo capital de cada empresa

  for (int i = 1; i <= n; i++)
    printf("%d ", empr[i]); // imprimo os valores no vetor
