...
bool compara(int x, int y) { // declaro a bool compara, que recebe dois inteiros
  ...
  if (nota[x] > nota[y])
    return true; // se a nota do primeiro for maior estão em ordem

  if (nota[y] > nota[x])
    return false; // se a nota do segundo for maior, não estão em ordem

  // se o programa chegar nessa linha, então as notas são iguais, então olho
  // para os nomes

  if (strcmp(nome[x], nome[y]) < 0)
    return true; // se o nome do prieiro vier antes, estão em ordem
...
int main() {
  ...
  for (int i = 1; i <= n; i++)
    scanf(" %s %lf", nome[i], &nota[i]); // leios os nomes e notas dos alunos

  for (int i = 1; i <= n; i++)
    aluno[i] = i; // faço o vetor aluno guardar os números de 1 a n

  sort(aluno + 1, aluno + n + 1,
       compara); // e o ordeno segundo a função compara
