...
void bubble_sort() { // declaro a função void buble_sort
  ...
  while (ordenado == 0) { // enquanto ordenado for 0
    ...
    for (int i = 1; i < n;
         i++) // e checo para todas as posições, exceto a última
      if (vetor[i] >
          vetor[i + 1]) { // se não há inversão entre vetor[i] e vetor[i+1]
...
int main() {
  ...
  for (int i = 1; i <= n; i++)
    scanf("%d", &vetor[i]); // leio os elementos do vetor
  ...
  for (int i = 1; i <= n; i++)
    printf(
        "%d ",
        vetor[i]); // imprimo os elementos do vetor, que agora estarão ordenados
